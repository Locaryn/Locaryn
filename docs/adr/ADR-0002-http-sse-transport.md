...
## Context
Lochor needs a client-facing API for the local daemon (loopback) and the remote-server (TLS). Both surfaces (desktop, CLI) must speak the same contract. Streaming is required for tokens, logs, task state, preview updates — all server→client. Rare bidirectional needs: preview live-reload, interactive shell.
...
- MCP transport is handled separately by `lochor-mcp` per the MCP 2026-07-28 spec (stateless HTTP + stdio) — not part of this decision.
