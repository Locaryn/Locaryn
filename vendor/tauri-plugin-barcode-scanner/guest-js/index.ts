...
export async function checkPermissions(): Promise<PermissionState> {
  return await checkPermissions_<{ camera: PermissionState }>("barcode-scanner").then(
    (r) => r.camera,
  );
}
...
export async function requestPermissions(): Promise<PermissionState> {
  return await requestPermissions_<{ camera: PermissionState }>("barcode-scanner").then(
    (r) => r.camera,
  );
}
