...
async fn main() -> anyhow::Result<()> {
    ...
    if exposed {
        ...
        if let Some(name) = cfg.daemon.travel.clone() {
            // Meme ecriture que par l'API : « ssh:moi@serveur.fr:8443 ».
            let (nom_relais, cible_brute) = match name.split_once(':') {
                Some((tete, reste)) => (tete.to_string(), Some(reste.to_string())),
                None => (name.clone(), None),
            };
            match locaryn_travel::Provider::parse(&nom_relais) {
                Some(p) => {
                    let cible = if p.needs_target() {
                        match locaryn_travel::SshTarget::parse(cible_brute.as_deref().unwrap_or(""))
                        {
                            Ok(t) => Some(t),
                            Err(e) => {
                                tracing::warn!("mode voyage indisponible — {e}");
                                travel_state.record_blocker(e).await;
                                return Ok(());
                            }
                        }
                    } else {
                    ...
                    match travel_state
                        .start(p, cible.as_ref(), port, &data_dir_for_tls, true)
                        .await
                    {
                        Ok(st) => {
...
async fn send_message(
    ...
    let consigne_choisie = match cfg.as_ref().and_then(|c| c.assistance.system_prompt.as_deref()) {
