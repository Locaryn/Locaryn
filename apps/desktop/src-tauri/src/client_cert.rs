...
/// token, so every later request rides the standard Bearer circuit.
#[tauri::command]
pub async fn confirm_pairing(
    ...
    let url = format!("{}/v1/auth/pair/confirm", server_url.trim_end_matches('/'));
    ...
    let resp = client
        ...
        .map_err(|e| {
            ...
            if chain.contains("empreinte") {
                "Le certificat présenté ne correspond pas à l'empreinte fournie par votre \
                 administrateur. Ne saisissez pas le code : signalez-le d'abord."
                    .to_string()
            } else {
                format!(
                    "Serveur injoignable ({e}). Vérifiez qu'il est allumé et que vous êtes \
    ...
    if resp.status() == reqwest::StatusCode::CONFLICT {
        return Err(
            "Aucun compte administrateur sur ce serveur : l'appairage par code exige un compte \
             à appairer."
                .into(),
        );
    }
