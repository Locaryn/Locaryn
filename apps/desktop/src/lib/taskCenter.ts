...
import { type NotificationGroup, core } from "./core";
 ...
 */
/**
 * Le groupe de notification auquel chaque type de tache appartient.
 *
 * Ce qui permet de couper les bannieres de telechargement sans perdre celles
 * des workflows : sans cette table, la seule alternative aurait ete de tout
 * couper.
 */
const GROUPE_PAR_TYPE: Record<TaskType, NotificationGroup> = {
  download: "downloads",
  generation: "long_tasks",
  edit: "long_tasks",
  workflow: "long_tasks",
  audio: "long_tasks",
};
...
/**
 * Reporte l'avancement sur l'icone de la barre des taches.
 *
 * Une seule barre pour toutes les taches : c'est celle qui avance le moins qui
 * s'affiche, parce qu'elle dit quand tout sera fini. Sans tache en cours, la
 * barre disparait plutot que de rester a cent pour cent — une barre pleine qui
 * ne bouge plus se lit comme un travail bloque.
 */
function refleterDansLaBarre(): void {
  const enCours = tasks.filter((t) => t.status === "running");
  if (enCours.length === 0) {
    const echoue = tasks.some((t) => t.status === "error" && !t.read);
    void core.setTaskbarProgress(echoue ? "error" : "none").catch(() => {});
    return;
  }
  const connus = enCours.map((t) => t.progress).filter((p): p is number => typeof p === "number");
  if (connus.length === 0) {
    // Rien de chiffre : la barre defile, ce qui dit « ca travaille » sans
    // pretendre savoir combien il reste.
    void core.setTaskbarProgress("indeterminate").catch(() => {});
    return;
  }
  void core.setTaskbarProgress("normal", Math.min(...connus)).catch(() => {});
}

/** La duree d'une tache, pour savoir si sa fin merite une banniere. */
function dureeDe(id: string): number | undefined {
  const t = tasks.find((x) => x.id === id);
  return t ? now() - t.createdAt : undefined;
}
...
export const taskCenter = {
  ...
  update(id: string, patch: Partial<Omit<AppTask, "id">>) {
    ...
    emit();
    if (patch.progress !== undefined || patch.status !== undefined) refleterDansLaBarre();
  },
  done(id: string, patch?: Partial<Omit<AppTask, "id" | "status">>) {
    const tache = tasks.find((t) => t.id === id);
    const duree = dureeDe(id);
    ...
    void notifierSysteme(
      "Terminé",
      tache?.label,
      tache ? GROUPE_PAR_TYPE[tache.type] : "long_tasks",
      duree,
    );
  },
  fail(id: string, error: string) {
    ...
    // cette bannière sans rouvrir l'application. Et il ne passe pas par le
    // seuil de durée : une tâche qui échoue en deux secondes est justement ce
    // qu'on veut savoir tout de suite.
    void notifierSysteme(
      "Échec",
      tache?.label ? `${tache.label} — ${error}` : error,
      tache ? GROUPE_PAR_TYPE[tache.type] : "long_tasks",
    );
  },
