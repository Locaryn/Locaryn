...
export interface BatchJob {
  id: string;
  name: string;
  provider: "OpenAI Batch (-50%)" | "Anthropic Batch (-50%)" | "DeepSeek Batch (-50%)" | "Ollama Local Batch";
...
export function BatchStudio() {
  ...
  const [selectedProvider, setSelectedProvider] = useState<BatchJob["provider"]>("DeepSeek Batch (-50%)");
  ...
  function handleCreateBatch() {
    ...
    const lines = batchFileText.trim().split("\n").filter((l) => l.trim().length > 0);
    ...
    setTimeout(() => {
      setJobs((prev) =>
        prev.map((j) => (j.id === newJ.id ? { ...j, completedRequests: Math.round(count * 0.5) } : j))
      );
    }, 2500);

    setTimeout(() => {
      setJobs((prev) =>
        prev.map((j) => (j.id === newJ.id ? { ...j, completedRequests: count, status: "completed" } : j))
      );
    }, 5000);
  }
    ...
    <div className="lochor-view-container" style={{ padding: "var(--space-4)", overflowY: "auto" }}>
      <div className="lochor-view-header">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <div style={{ display: "flex", gap: "12px", background: "var(--surface)", padding: "8px 16px", borderRadius: "var(--radius)", border: "1px solid var(--border)" }}>
            <div>
              <span style={{ fontSize: "10px", color: "var(--text-faint)", display: "block" }}>ÉCONOMIE JETONS</span>
              <strong style={{ color: "#64c878", fontSize: "14px" }}>{totalSavedTokens.toLocaleString()} tokens</strong>
            </div>
            <div style={{ borderLeft: "1px solid var(--border)", paddingLeft: "12px" }}>
              <span style={{ fontSize: "10px", color: "var(--text-faint)", display: "block" }}>RÉDUCTION COÛT</span>
              <strong style={{ color: "var(--accent)", fontSize: "14px" }}>-${totalSavedUsd.toFixed(2)} USD (-50%)</strong>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="lochor-box-card" style={{ marginTop: "16px", padding: "16px" }}>
        ...
        <div style={{ marginBottom: "12px" }}>
          <label style={{ fontSize: "11px", color: "var(--text-faint)", marginBottom: "4px", display: "block" }}>
