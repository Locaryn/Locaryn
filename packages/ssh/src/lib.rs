...
//!
//! Security notes:
//! * Host keys are TOFU-pinned: `connect` with `pinned = Some(fp)` fails hard
//!   if the presented fingerprint differs (possible MITM). With `pinned = None`
//!   the fingerprint is captured for the caller to confirm before saving.
//! * Secrets are held in `Zeroizing` and never logged.
